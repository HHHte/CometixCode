//! Maps to CC `tools/TeamCreateTool/constants.ts` and `prompt.ts`.

pub const TEAM_CREATE_TOOL_NAME: &str = "TeamCreate";

/// Maps to: CC `TeamCreateTool.ts:107-109` `description()`.
pub const DESCRIPTION: &str = "Create a new team for coordinating multiple agents";

/// Maps to: CC `TeamCreateTool/prompt.ts` `getPrompt()`.
