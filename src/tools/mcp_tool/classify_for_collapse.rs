...
//!
//! Maps to: CC `tools/MCPTool/classifyForCollapse.ts`.

#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub struct McpToolCollapseClassification {
    pub is_search: bool,
    pub is_read: bool,
}
...
fn normalize_mcp_tool_name(name: &str) -> String {
    // Maps to CC `normalize(name)`: insert `_` for lower→upper camelCase
    // boundaries, convert kebab-case to snake_case, then lowercase.
    ...
    for ch in name.chars() {
        ...
        if previous_was_lowercase && ch.is_ascii_uppercase() {
            normalized.push('_');
        }
...
/// Maps to: CC `classifyMcpToolForCollapse(_serverName, toolName)`.
pub fn classify_mcp_tool_for_collapse(
    _server_name: &str,
    tool_name: &str,
) -> McpToolCollapseClassification {
    let normalized = normalize_mcp_tool_name(tool_name);
    McpToolCollapseClassification {
...
mod tests {
    #[test]
    fn mcp_tool_collapse_classification_normalizes_like_official() {
        assert!(super::classify_mcp_tool_for_collapse("github", "searchRepositories").is_search);
        assert!(super::classify_mcp_tool_for_collapse("notion", "browser-route-list").is_read);
        assert!(super::classify_mcp_tool_for_collapse("slack", "slack_read_channel").is_read);
        assert!(!super::classify_mcp_tool_for_collapse("slack", "slack_post_message").is_search);
        assert!(!super::classify_mcp_tool_for_collapse("slack", "slack_post_message").is_read);
    }
}
