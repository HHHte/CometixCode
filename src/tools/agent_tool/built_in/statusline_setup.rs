//! Maps to CC `tools/AgentTool/built-in/statuslineSetup.ts`.

use crate::tools::agent_tool::load_agents_dir::{AgentDefinition, AgentDefinitionSource};

pub const STATUSLINE_SETUP_WHEN_TO_USE: &str =
    "Use this agent to configure the user's Claude Code status line setting.";

/// Maps to CC `statuslineSetup.ts#STATUSLINE_SYSTEM_PROMPT`.
pub const STATUSLINE_SYSTEM_PROMPT: &str = r##"You are a status line setup agent for Claude Code. Your job is to create or update the statusLine command in the user's Claude Code settings.
...
/// Maps to CC `statuslineSetup.ts#STATUSLINE_SETUP_AGENT`.
pub fn statusline_setup_agent() -> AgentDefinition {
    let mut agent = AgentDefinition::new(
        "statusline-setup",
        STATUSLINE_SETUP_WHEN_TO_USE,
    ...
    // Maps to: CC `statuslineSetup.ts:142` `color: 'orange'`.
    agent.color = Some("orange".to_string());
    agent.system_prompt = Some(STATUSLINE_SYSTEM_PROMPT.to_string());
    agent
}
...
mod tests {
    ...
    #[test]
    fn statusline_setup_agent_matches_official_metadata() {
        ...
        assert!(
            agent
                .system_prompt
                .as_deref()
                .is_some_and(|prompt| prompt.contains("statusLine command"))
        );
    }
}
