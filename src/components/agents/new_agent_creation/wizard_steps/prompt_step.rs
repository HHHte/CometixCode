//! Maps to: CC `components/agents/new-agent-creation/wizard-steps/PromptStep.tsx:1-97`.
...
#[component]
pub fn PromptStep(mut hooks: Hooks) -> impl Into<AnyElement<'static>> {
    let wizard = use_wizard(&mut hooks);
    let initial = string(&wizard.wizard_data, "systemPrompt").unwrap_or_default();
    ...
    if let Some(submitted) = submitted {
        ...
        if trimmed.is_empty() {
            error.set(Some("System prompt is required".to_string()));
        } else {
            error.set(None);
            wizard.update_wizard_data(update("systemPrompt", json!(trimmed)));
            wizard.go_next();
        }
    }
    let runtime = hooks
    ...
    element! {
        WizardDialogLayout(
            subtitle: Some("System prompt".to_string()),
            footer_text: Some("Type to enter text · Enter to continue · Ctrl+G to open in editor · Esc to go back".to_string()),
        ) {
            View(flex_direction: FlexDirection::Column) {
                Text(content: "Enter the system prompt for your agent:".to_string())
                Text(content: "Be comprehensive for best results".to_string(), dim: true)
                View(margin_top: 1u32) {
                    TextInput(
                        value: value, cursor_offset: cursor, focus: true, show_cursor: true, columns: 80usize,
                        placeholder: Some("You are a helpful code reviewer who...".to_string()),
